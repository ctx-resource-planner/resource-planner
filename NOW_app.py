...
# Report utilities
from utils.reports import generate_excel_report, send_report_email
...
@app.route('/download-allocations-excel')
@login_required
def download_allocations_excel():
    """Generate and download allocations report as Excel"""
    query = Allocation.query.join(Employee).join(Project).order_by(
        Employee.name, Project.name, Allocation.start_date
    )
    return generate_excel_report(
        query=query,
        filename_prefix='allocations_report',
        columns=['employee_name', 'project_name', 'billable_allocation_percentage', 'start_date', 'end_date']
    )

@app.route('/email-allocations-report')
@login_required
def email_allocations_report():
    """Email allocations report to the current user"""
    query = Allocation.query.join(Employee).join(Project).order_by(
        Employee.name, Project.name, Allocation.start_date
    )
    success, error = send_report_email(
        query=query,
        template_name='allocations',
        subject='Allocations Report',
