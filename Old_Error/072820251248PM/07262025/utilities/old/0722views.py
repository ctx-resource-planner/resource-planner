...
def project_setup():
    if request.method == 'POST':
        try:
            ...
            customer = process_customer_data(form_data)
            
            # Process project data
            project = process_project_data(form_data, customer)
            
            # Process employee data
            employees = process_employee_data(form_data)
            
            # Save all changes
            db.session.commit()
            
            # Store in session for preview/email
            ...
            flash('An error occurred while saving the project. Please try again.', 'danger')
            return redirect(url_for('utilities.project_setup'))
...
def process_customer_data(form_data):
    ...
    if not customer:
        customer = Customer(
            ...
            customer_address_1=form_data.get('customer_address', [''])[0],
            bill_contact_name=form_data.get('customer_bill_contact_name', [''])[0],
            bill_contact_email=form_data.get('customer_bill_contact_email', [''])[0],
            bill_contact_phone=form_data.get('customer_bill_contact_phone', [''])[0]
        )
...
def process_project_data(form_data, customer):
    """Process and save project data"""
    project = Project(
        name=form_data['project_name_quickbooks'][0],
        project_type=form_data['project_type'][0],
        customer=customer,
        start_date=datetime.strptime(form_data.get('project_start_date', [None])[0], '%Y-%m-%d') if form_data.get('project_start_date', [''])[0] else None,
        end_date=datetime.strptime(form_data.get('estimated_end_date', [None])[0], '%Y-%m-%d') if form_data.get('estimated_end_date', [''])[0] else None,
        status=form_data.get('project_status', [''])[0],
        po_number=form_data.get('customer_po_number', [''])[0],
        gross_margin=float(form_data.get('as_bid_gross_margin', [0])[0] or 0),
        estimating_sheet_link=form_data.get('project_estimating_sheet_link', [''])[0],
...
def process_employee_data(form_data):
    """Process and save employee assignments"""
    employees = []
    ...
    for i, emp_name in enumerate(employee_names):
        if emp_name == 'new' and i < len(new_employee_names):
            # Create new employee
            employee = Employee(
                name=new_employee_names[i],
                email=new_employee_emails[i] if i < len(new_employee_emails) else '',
                is_active=True
            )
            db.session.add(employee)
        else:
            # Use existing employee
            employee = Employee.query.filter_by(email=emp_name).first()
        ...
        if employee:
            ...
            assignment = EmployeeAssignment(
                employee=employee,
                project_id=project.id,
                role=form_data.get('employee_role[]', [''])[i] if i < len(form_data.get('employee_role[]', [])) else '',
                rate=float(form_data.get('employee_rate[]', [0])[i] or 0) if i < len(form_data.get('employee_rate[]', [])) else 0,
                start_date=datetime.strptime(form_data.get('employee_start_date[]', [None])[i], '%Y-%m-%d') if i < len(form_data.get('employee_start_date[]', [])) and form_data.get('employee_start_date[]', [''])[i] else None,
                end_date=datetime.strptime(form_data.get('employee_end_date[]', [None])[i], '%Y-%m-%d') if i < len(form_data.get('employee_end_date[]', [])) and form_data.get('employee_end_date[]', [''])[i] else None
            )
