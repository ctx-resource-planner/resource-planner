...
def index():
    try:
        ...
        # Handle search if it's a POST request
        if request.method == 'POST':
            search_type = request.form.get('search_type', 'skill')
            search_query = request.form.get('search_query', '').strip().lower()
            
            if search_type == 'skill' and search_query:
                # Search by skill
                results = Employee.query\
                    .join(Employee.skill_associations)\
                    .join(EmployeeSkill.skill)\
                    .filter(func.lower(Skill.name).ilike(f'%{search_query}%'))\
                    .filter(Employee.is_active == True)\
                    .distinct()\
                    .all()
            
            elif search_type == 'employee' and search_query:
                # Search by employee name
                results = Employee.query\
                    .filter(func.lower(Employee.name).ilike(f'%{search_query}%'))\
                    .filter(Employee.is_active == True)\
                    .all()
        ...
        return render_template('index.html',
                            ...
                            search_criteria={'search_type': 'skill'},
                            allocation_date_headers={},
                            selected_project_with_skills=None)@app.route('/projects/combined/<int:project_id>')
@login_required
...
def employees_list():
    try:
        ...
        app.logger.error(f"Error in employees_list: {str(e)}", exc_info=True)
...
def index():
    try:
        ...
        # Get active employees with their skills
        employees = Employee.get_active().options(
            joinedload(Employee.skill_associations).joinedload(EmployeeSkill.skill)
        ).all()
