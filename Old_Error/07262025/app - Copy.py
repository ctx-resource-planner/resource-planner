...
##db = SQLAlchemy(app)
mail = Mail(app)# Standard Library Imports
import os
...
def projects_list():
    sort_by_param = request.args.get('sort_by', 'name'); order_param = request.args.get('order', 'asc')
    query = Project.query
    if hasattr(Project, sort_by_param): column_to_sort = getattr(Project, sort_by_param); query = query.order_by(desc(column_to_sort) if order_param == 'desc' else asc(column_to_sort))
    else: query = query.order_by(asc(Project.name))
    try: projects_data = query.all()
    except Exception as e: flash(f'Error fetching projects: {e}', 'danger'); projects_data = []; app.logger.error(f"Error fetching projects: {e}", exc_info=True)
    return render_template('projects_list.html', projects=projects_data, sort_by=sort_by_param, current_order=order_param)
...
def allocations_list():
    ...
    elif sort_by_param == 'allocation_percentage': query = query.order_by(desc(Allocation.billable_allocation_percentage) if order_param == 'desc' else asc(Allocation.billable_allocation_percentage))
...
def view_project(project_id):
    ...
    # Get fixed price items if they exist in project_setup
    fixed_price_items = []
    if hasattr(project_setup, 'employee_contractor_details') and isinstance(project_setup.employee_contractor_details, dict):
    ...
    <h2>Team Members</h2>
    <ul>
        {"".join(f"<li>{m['name']} ({m['email']}) - {m['allocation']}%</li>" for m in team_members)}
    </ul>
    <h2>Fixed Price Items</h2>
    <ul>
        {"".join(f"<li>{item.get('description', 'No description')}: ${item.get('amount', 0):.2f}</li>" for item in fixed_price_items)}
    </ul>
    """
    return html
...
###################### temp #########################
# Simple test route - add this to app.py
@app.route('/test_route')
def test_route():
    return "Test route is working!"
    
@app.route('/test_view/<int:project_id>')
@login_required
def test_view(project_id):
    ...
    if not project_setup:
        return "Project setup not found", 404
