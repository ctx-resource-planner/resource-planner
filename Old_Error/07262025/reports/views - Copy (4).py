...
def executive_utilization_dashboard():
    """Executive Utilization Dashboard - SIMPLIFIED VERSION"""
    try:
        import pandas as pd
        ...
        # Process each employee - SIMPLIFIED
        rows = []
        ...
        <table id="utilizationTable" class="table table-bordered table-striped table-sm">
            ...
            {% for row in rows %}
                <tr>
                    ...
                    <td><b>{{ "%.0f"|format((month_data.get('billable_hours', 0) / 22 * 100) if month_data.get('billable_hours', 0) > 0 else 0) }}%</b></td>
                    <td><b>{{ "%.0f"|format(month_data.get('nonbillable_hours', 0)) }}</b></td>
                    <td><b>{{ "%.0f"|format((month_data.get('nonbillable_hours', 0) / 22 * 100) if month_data.get('nonbillable_hours', 0) > 0 else 0) }}%</b></td>
                    {% endfor %}
                </tr>
            {% endfor %}
            </tbody>
...
function emailReport() {
    ...
    if (email) {
        alert('Email functionality - to be implemented for: ' + email);
    }
}
</script>
</body>
