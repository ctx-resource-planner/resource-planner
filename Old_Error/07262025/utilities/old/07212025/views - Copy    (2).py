...
"""
        ...
        msg.attach(filename, "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet", excel_data.read())
        mail.send(msg)
...
def generate_project_setup_excel(project_setup, form_data):
    ...
    with pd.ExcelWriter(output, engine='openpyxl') as writer:
        # Project Details
        pd.DataFrame({
            'Field': ['Company', 'Customer', 'Project Name', 'Start Date', 'End Date'],
            'Value': [
                project_setup.company_name,
                project_setup.customer_name,
                project_setup.project_name_quickbooks,
                project_setup.project_start_date,
                project_setup.estimated_end_date
            ]
        }).to_excel(writer, sheet_name='Project Details', index=False)
        
        # Employee Details
        if project_setup.employee_contractor_details:
            pd.DataFrame(project_setup.employee_contractor_details).to_excel(
                writer, sheet_name='Team', index=False)
        
        # Fixed Price Details
        if project_setup.fixed_price_details:
            pd.DataFrame(project_setup.fixed_price_details).to_excel(
                writer, sheet_name='Fixed Price', index=False)
