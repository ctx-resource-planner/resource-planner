        ...
        try:
            if request.form.get('action') == 'edit':
                ...
                try:
                        ...
                        for i in range(len(employee_data['names'])):
                            ...
                            if employee:
                                allocation = Allocation(
                                    ...
                                    billable_allocation_percentage=100,
                    ...
                    # Generate and send email with PDF
                    try:
                        pdf_buffer = generate_project_pdf(form_data)
                        send_project_email(form_data, pdf_buffer)
...
def generate_project_pdf(form_data):
    """Generate a clean PDF report for the project"""
    buffer = BytesIO()
    doc = SimpleDocTemplate(buffer, pagesize=letter)
    elements = []
    styles = getSampleStyleSheet()
...
def send_project_email(form_data, pdf_buffer):
    ...
    try:
        ...
        project_name = form_data.get('project_name_quickbooks', 'New Project')
        
        msg = Message(
            subject=f"Project Setup Confirmation - {project_name}",
            sender=current_app.config.get('MAIL_DEFAULT_SENDER'),
            recipients=recipients
        )
        ...
        <html>
            <body>
                <h2>Project Setup Confirmation</h2>
                <p>Project <strong>{project_name}</strong> has been successfully set up.</p>
                <p>Please find the attached PDF with all project details.</p>
        ...
        The project has been successfully set up. Please find the attached PDF with all details.
        ...
        mail.send(msg)
        current_app.logger.info(f"Project setup email sent to {recipients}")
        return True
        
    except Exception as e:
        current_app.logger.error(f"Failed to send project email: {str(e)}")
        return False
