...
from models import Project, ProjectSetup, Employee, Allocation, BusinessUnit, Customer
                ...
                for i in range(fp_count):
                    ...
                    if desc:  # Only add if description exists
                        fixed_price_data.append({
                            'description': desc,
                            'amount': float(request.form.getlist('fp_amount[]')[i]) if request.form.getlist('fp_amount[]')[i] else 0.0
                        })
                ...
                for emp_data in employee_data:
                    ...
                    if employee:
                        ...
                        allocation = Allocation(
                            ...
                            bill_rate=emp_data.get('bill_rate'),
                            cost_rate=emp_data.get('cost_rate')
                        )
...
def project_setup_preview():
    ...
    # Get business unit for display
    business_unit = None
    if form_data.get('business_unit_id'):
        business_unit = BusinessUnit.query.get(form_data['business_unit_id'])
...
def send_project_email_with_pdf(form_data):
    ...
    try:
        # Generate PDF first
        from utilities.views import generate_simple_pdf
        pdf_buffer = generate_simple_pdf(form_data)
