...
##from app import db  # adjust import if your db instance is elsewhere
...
class Employee(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    empid = db.Column(db.String(50), unique=True, nullable=True)
    name = db.Column(db.String(150), nullable=False)
    email = db.Column(db.String(255), unique=True, nullable=True)
    service_line = db.Column(db.String(100), nullable=True)
    is_active = db.Column(db.Boolean, default=True, nullable=False)
    doj = db.Column(db.Date)  # Date of Joining
    doe = db.Column(db.Date)  # Date of Exit
    reporting_manager = db.Column(db.String(100))
    designation = db.Column(db.String(100))
    location = db.Column(db.String(100))
    skill_associations = db.relationship("EmployeeSkill", back_populates="employee", cascade="all, delete-orphan", lazy="joined")
...
class Customer(db.Model):
    ...
    # Relationships
    #projects = db.relationship('Project', back_populates='customer', cascade="all, delete-orphan")
    
    def __repr__(self):
        return f'<Customer {self.customer_name}>'
        
# Add this field to your existing Project model
customer_id = db.Column(db.Integer, db.ForeignKey('customers.id'), nullable=True)
customer = db.relationship("Customer", back_populates="projects")

# Also add these fields to match ProjectSetup form
...
class ProjectSetup(db.Model):
    ...
    project = db.relationship("Project", backref="project_setup")
