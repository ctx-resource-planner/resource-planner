...
from flask import (
    ...
    url_for, flash, session, jsonify, current_app, send_file
)
...
#from app.extensions import db, mail  # Assuming you have an extensions.py
from models import (
    Employee, ProjectSetup, Customer, 
    Project, Allocation, BusinessUnit
)


##@utilities_bp.route('/test')
##def test_route():
...
#####################################################
@utilities_bp.route('/preview_project_email/<int:project_id>', methods=['POST'])
@login_required
@admin_required
def preview_project_email(project_id):
    """Generate a preview of the project setup email"""
    try:
        # Get the additional message from the request
        data = request.get_json()
        additional_message = data.get('message', '')
        
        # Get project details
        project = Project.query.get_or_404(project_id)
        project_setup = ProjectSetup.query.filter_by(project_id=project_id).first()
        
        # Generate a preview of the email content
        email_preview = f"""
        <div style='font-family: Arial, sans-serif; max-width: 600px; margin: 0 auto;'>
            <h2>Project Setup: {project.name}</h2>
            <p><strong>Customer:</strong> {project_setup.customer_name if project_setup else 'N/A'}</p>
            <p><strong>Project Type:</strong> {project.project_type}</p>
            <p><strong>Start Date:</strong> {project.sow_start_date.strftime('%Y-%m-%d') if project.sow_start_date else 'N/A'}</p>
            <p><strong>End Date:</strong> {project.sow_end_date.strftime('%Y-%m-%d') if project.sow_end_date else 'N/A'}</p>
            <p><strong>Status:</strong> {project_setup.project_status if project_setup else 'Active'}</p>
        """
        
        if additional_message:
            email_preview += f"<div style='margin-top: 20px; padding: 10px; background-color: #f8f9fa; border-left: 4px solid #007bff;'>"
            email_preview += f"<p><strong>Additional Message:</strong></p>"
            email_preview += f"<p>{additional_message}</p>"
            email_preview += "</div>"
...
@admin_required
def project_setup():
    # Initialize form data and dropdowns
    customers = Customer.query.order_by(Customer.customer_name).all()
    form_data = request.form.to_dict() if request.method == 'POST' else {}
    employees = Employee.query.filter_by(is_active=True).order_by(Employee.name).all()
    business_units = BusinessUnit.query.order_by(BusinessUnit.name).all()
    customers = Customer.query.order_by(Customer.customer_name).all()
    project_id = None
    employee_data = []
    fixed_price_data = []
    ...
    if request.method == 'POST':
        try:
            ...
            project_name = form_data.get('project_name_quickbooks')
            if project_name:
                existing_project = ProjectSetup.query.filter(
                    ProjectSetup.project_name_quickbooks.ilike(project_name)
                ).first()
                
                if existing_project and (not request.form.get('project_id') or 
                                       str(existing_project.id) != request.form.get('project_id')):
                    flash(f"A project with the name '{project_name}' already exists. Please choose a different name.", 'error')
                    return render_template(
                        'utilities/project_setup.html',
                        form_data=form_data,
                        employees=employees,
            ...
            if 'employee_name[]' in request.form:
                employee_count = len(request.form.getlist('employee_name[]'))
                for i in range(employee_count):
                    employee_data.append({
                        'email': request.form.getlist('employee_name[]')[i],
                        'onshore_offshore': request.form.getlist('onshore_offshore[]')[i],
                        'tsheet_service': request.form.getlist('tsheet_service_name[]')[i],
                        'bill_rate': float(request.form.getlist('bill_rate[]')[i]) if request.form.getlist('bill_rate[]')[i] else 0.0,
                        'cost_rate': float(request.form.getlist('cost_rate[]')[i]) if request.form.getlist('cost_rate[]')[i] else 0.0
                    })
            
            # Process fixed price data
            fixed_price_data = []
            if 'fp_description[]' in request.form:
                fp_count = len(request.form.getlist('fp_description[]'))
                for i in range(fp_count):
                    desc = request.form.getlist('fp_description[]')[i]
                    amount = request.form.getlist('fp_amount[]')[i]
                    if desc or amount:  # Only add if either description or amount exists
                        fixed_price_data.append({
                            'description': desc,
                            'amount': float(amount) if amount else 0.0
                        })
            
            # Add employee and fixed price data to form_data
            form_data['employee_data'] = employee_data
            form_data['fixed_price_data'] = fixed_price_data
            
            # Set default email recipients if not provided
            if 'email_recipients' not in form_data or not form_data['email_recipients']:
                form_data['email_recipients'] = f"ravindra.sonakiya@clovertex.com, {current_user.email}"
            ...
            try:
                ...
                project_data = {
                    ...
                    'service_line': form_data.get('business_unit_id'),
                    ...
                    'po_amount': float(form_data.get('customer_po_number')) if form_data.get('customer_po_number') and form_data.get('customer_po_number').replace('.', '', 1).isdigit() else None
                }
                ...
                if request.form.get('project_id'):
                    ...
                    Allocation.query.filter_by(project_id=project.id).delete()
                else:
                    # Create new project
                ...
                # 3. Create or update ProjectSetup record
                project_setup_data = {
                    'project_id': project.id,
                    'company_name': form_data.get('customer_name'),
                    ...
                    'project_status': form_data.get('project_status', 'Active'),
                    ...
                    'business_unit_id': form_data.get('business_unit_id') or None,
                    'employee_contractor_details': {
                        'team_members': employee_data,
                        'fixed_price_items': fixed_price_data  # Store fixed price items here
                    }
                }
                if request.form.get('project_id'):
                    # Update existing project setup
                    project_setup = ProjectSetup.query.filter_by(project_id=project.id).first()
                    if project_setup:
                        for key, value in project_setup_data.items():
                            if key != 'project_id' and value is not None:
                                setattr(project_setup, key, value)
                        project_setup.updated_by = current_user.username
                        project_setup.updated_at = datetime.utcnow()
                else:
                    # Create new project setup
                    project_setup = ProjectSetup(**{k: v for k, v in project_setup_data.items() if v is not None})
                    project_setup.created_by = current_user.username
                ...
                # Ensure we have the latest business_units data
                business_units = BusinessUnit.query.order_by(BusinessUnit.name).all()
...
@utilities_bp.route('/project/<int:project_id>/edit', methods=['GET', 'POST'])
@login_required
@admin_required
def edit_project(project_id):
    ...
    if request.method == 'POST':
        # Process the form submission using the project_setup function
        return project_setup(project_id=project_id)
    ...
    if not project_setup:
        flash('No project setup data found for this project', 'error')
        return redirect(url_for('utilities.project_setup'))
    ...
    for alloc in allocations:
        emp = Employee.query.get(alloc.employee_id)
        if emp:
            employee_assignments.append({
                'email': emp.email,
                'onshore_offshore': 'Onshore',  # Default value, adjust as needed
                'tsheet_service': emp.designation or '',  # Default value
                'bill_rate': 0,  # Default value
                'cost_rate': 0   # Default value
            })
    
    # Get employee data from project_setup if available
    employee_data = []
    fixed_price_data = []
    
    if project_setup.employee_contractor_details:
        if isinstance(project_setup.employee_contractor_details, dict):
            employee_data = project_setup.employee_contractor_details.get('team_members', [])
            fixed_price_data = project_setup.employee_contractor_details.get('fixed_price_items', [])
        else:
            # Handle case where employee_contractor_details is not a dictionary
            employee_data = project_setup.employee_contractor_details or []
    
    # Prepare form data from project and project_setup
    form_data = {
        'project_name_quickbooks': project.name,
        'customer_name': project_setup.customer_name,
        'customer_abbreviation': project_setup.customer_abbreviation,
        'customer_address': project_setup.customer_address,
        'customer_bill_contact_name': project_setup.customer_bill_contact_name,
        'customer_bill_contact_email': project_setup.customer_bill_contact_email,
        'customer_bill_contact_phone': project_setup.customer_bill_contact_phone,
        'customer_po_number': project_setup.customer_po_number,
        'project_type': project_setup.project_type or 'Time & Materials',  # Default to Time & Materials if None
        'project_start_date': project.sow_start_date.strftime('%Y-%m-%d') if project.sow_start_date else '',
        'estimated_end_date': project.sow_end_date.strftime('%Y-%m-%d') if project.sow_end_date else '',
        'project_status': project_setup.project_status or 'Active',  # Default to Active if None
        'as_bid_gross_margin': project_setup.as_bid_gross_margin,
        'project_estimating_sheet_link': project_setup.project_estimating_sheet_link,
        'email_recipients': project_setup.email_recipients or current_user.email,
        'business_unit_id': project.service_line,
        'employee_data': employee_data or employee_assignments,
        'fixed_price_data': fixed_price_data
    }
    
    # Validate project type
    if form_data['project_type'] not in ['Fixed Price', 'Time & Materials', 'Non-Billable']:
        form_data['project_type'] = 'Time & Materials'  # Default to Time & Materials if invalid
    
    # Validate project status
    if form_data['project_status'] not in ['Active', 'Inactive', 'On Hold', 'Completed']:
        form_data['project_status'] = 'Active'  # Default to Active if invalid
...
####################################################
@utilities_bp.route('/email_project_setup/<int:project_id>', methods=['POST'])
@admin_required
def email_project_setup(project_id):
    """Send project setup email with PDF attachment"""
    try:
        # Get form data
        ...
        if not project_setup:
            return jsonify({
                'success': False,
                'message': 'Project setup details not found'
            }), 404
        ...
        form_data = {
            ...
            'project_start_date': project.sow_start_date.strftime('%Y-%m-%d') if project.sow_start_date else '',
            'estimated_end_date': project.sow_end_date.strftime('%Y-%m-%d') if project.sow_end_date else '',
            'project_status': project_setup.project_status,
            'as_bid_gross_margin': project_setup.as_bid_gross_margin,
            'project_estimating_sheet_link': project_setup.project_estimating_sheet_link,
        ...
        # Send email
        msg = Message(
            subject=f"Project Setup: {project.name}",
            sender=current_app.config.get('MAIL_DEFAULT_SENDER'),
            recipients=[recipient_email]
        )
        
        # Email body
        msg.body = f"""Project: {project.name}
Customer: {project_setup.customer_name}
Type: {project.project_type}
Start Date: {project.sow_start_date.strftime('%Y-%m-%d') if project.sow_start_date else 'N/A'}
End Date: {project.sow_end_date.strftime('%Y-%m-%d') if project.sow_end_date else 'N/A'}
Status: {project_setup.project_status}

{message if message else 'Please find attached the project setup details.'}
...
"""
        ...
        # Attach PDF
        msg.attach(
            f"project_{project.name.replace(' ', '_')}_setup.pdf",
            "application/pdf",
            pdf_buffer.getvalue()
        )
        ...
        current_app.logger.error(f"Error sending project setup email: {str(e)}")
        return jsonify({
            'success': False,
...
def send_project_email_with_pdf(form_data):
    """Send email with PDF attachment in a clean, professional format"""
    try:
        # Generate PDF first
        pdf_buffer = generate_simple_pdf(form_data)
...
Please find attached the project details in PDF format.
...
"""
        ...
        # Reset buffer position to start before attaching
...
###############################################################################################
def get_combined_project_data(project_id):
    """
    Combines data from Project, ProjectSetup, and related tables without modifying existing code.
    Returns a dictionary with all project data.
    """
    from datetime import datetime
    
    # Get project and setup data using existing queries
    project = Project.query.get_or_404(project_id)
    project_setup = ProjectSetup.query.filter_by(project_id=project_id).first()
    ...
    result = {
        'project': project.to_dict() if hasattr(project, 'to_dict') else dict(project.__dict__),
        'project_setup': project_setup.to_dict() if hasattr(project_setup, 'to_dict') else dict(project_setup.__dict__),
        'team_members': [],
        'fixed_price_items': []
    }
    
    # Get employee data from allocations
    allocations = Allocation.query.filter_by(project_id=project_id).all()
    for alloc in allocations:
        emp = Employee.query.get(alloc.employee_id)
        if emp:
            result['team_members'].append({
                ...
                'allocation_percentage': alloc.billable_allocation_percentage,
                'start_date': alloc.start_date,
                'end_date': alloc.end_date
            })
    
    # Get fixed price items from employee_contractor_details if available
    if (hasattr(project_setup, 'employee_contractor_details') and 
        isinstance(project_setup.employee_contractor_details, dict)):
        result['fixed_price_items'] = project_setup.employee_contractor_details.get('fixed_price_items', [])
    
    # Clean up any SQLAlchemy instance state
    if '_sa_instance_state' in result['project']:
        del result['project']['_sa_instance_state']
    if '_sa_instance_state' in result['project_setup']:
        del result['project_setup']['_sa_instance_state']
    
    return result
    
#@utilities_bp.route('/project/<int:project_id>/combined', methods=['GET'])
#@login_required
#def view_combined_project(project_id):
 #   """Test route to view combined project data"""
 #   combined_data = get_combined_project_data(project_id)
...
def view_combined_project(project_id):
    ...
    return render_template('utilities/view_combined.html', project_data=combined_data)
    
@utilities_bp.route('/test_route')
def test_route():
    return "Test route is working!"
...
##@login_required
##def index():
   ## """Utilities dashboard page."""
   ## return render_template('utilities/index.html')

@utilities_bp.route('/clone_tsheet_upload')
@login_required
def clone_tsheet_upload():
    """Cloned timesheet upload utility."""
