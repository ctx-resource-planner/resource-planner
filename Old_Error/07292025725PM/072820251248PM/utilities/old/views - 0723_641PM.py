...
def send_project_creation_email(project):
    ...
    msg = Message(
        ...
        recipients=[current_user.email]  # Or your admin email
    )
    ...
    - PO Amount: ${project.po_amount:,.2f if project.po_amount else 'N/A'}
    - SOW Allocation FTE: {project.sow_allocation_fte or 'N/A'}
    
    Actual Details:
    - Start Date: {project.actual_start_date.strftime('%Y-%m-%d') if project.actual_start_date else 'N/A'}
    - End Date: {project.actual_end_date.strftime('%Y-%m-%d') if project.actual_end_date else 'N/A'}
    - Actual Allocation FTE: {project.actual_allocation_fte or 'N/A'}
    ...
    # Generate PDF
    pdf = generate_project_pdf(project)
    msg.attach(
...
def generate_project_pdf(project):
    # Render the same template as the review page
    html = render_template('utilities/project_setup_summary_pdf.html', project=project)
    
    # Generate PDF
    pdf = HTML(string=html).write_pdf(stylesheets=[
        CSS(string='''
            body { font-family: Arial, sans-serif; }
            .card { border: 1px solid #ddd; border-radius: 4px; padding: 20px; margin: 10px; }
            h2 { color: #2c3e50; }
