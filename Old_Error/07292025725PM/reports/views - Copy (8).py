...
@reports_bp.route('/executive-utilization-dashboard/export')
@login_required
...
@reports_bp.route('/executive-utilization-dashboard/email', methods=['POST'])
@login_required
...
<head>
    ...
    <style>
        ...
        /* Fixed Columns Styling - FIX INVISIBLE TEXT */
        .employee-name, .employee-bu {
            text-align: left !important;
            font-weight: 600;
            min-width: 120px;
            background: white !important;
            color: #333 !important;  /* ADDED: Dark text color */
            position: sticky;
            z-index: 5;
...
function exportToExcel() {
    var startMonth = $('#startMonth').val();
    var endMonth = $('#endMonth').val();
    var exportUrl = '/reports/executive-utilization-dashboard/export?start_month=' + startMonth + '&end_month=' + endMonth;
...
function emailReport() {
    ...
    if (email) {
        ...
        var form = $('<form method="post" action="/reports/executive-utilization-dashboard/email">');
        form.append('<input type="hidden" name="recipient_email" value="' + email + '">');
