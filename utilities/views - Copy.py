    ...
    try:
        ...
        <head>
            <style>
                ...
                .header {{ color: #2c3e50; border-bottom: 1px solid #eee; padding-bottom: 10px; margin-bottom: 20px; }}
        ...
        <body>
            <div class="header">
                <h2>Project Setup: {project_name}</h2>
            </div>
            
            <div class="section">
                <h3>Project Information</h3>
                <table>
                    <tr><td><strong>Project:</strong></td><td>{form_data.get('project_name_quickbooks', 'N/A')}</td></tr>
                    <tr><td><strong>Customer:</strong></td><td>{form_data.get('customer_name', 'N/A')}</td></tr>
                    <tr><td><strong>Type:</strong></td><td>{form_data.get('project_type', 'N/A')}</td></tr>
                    <tr><td><strong>Status:</strong></td><td>{form_data.get('project_status', 'Draft')}</td></tr>
                    <tr><td><strong>Start Date:</strong></td><td>{form_data.get('project_start_date', 'N/A')}</td></tr>
                    <tr><td><strong>End Date:</strong></td><td>{form_data.get('estimated_end_date', 'N/A')}</td></tr>
                    <tr><td><strong>PO Number:</strong></td><td>{form_data.get('customer_po_number', 'N/A')}</td></tr>
                    <tr><td><strong>Gross Margin:</strong></td><td>{form_data.get('as_bid_gross_margin', 'N/A')}</td></tr>
                </table>
            </div>
        """

        # Add billing info if available
        if any(form_data.get(field) for field in ['customer_bill_contact_name', 'customer_bill_contact_email', 'customer_bill_contact_phone']):
            html_content += """
            <div class="section">
                <h3>Billing Information</h3>
                <table>
            """
            if form_data.get('customer_bill_contact_name'):
                html_content += f"<tr><td><strong>Contact:</strong></td><td>{form_data['customer_bill_contact_name']}</td></tr>"
            if form_data.get('customer_bill_contact_email'):
                html_content += f"<tr><td><strong>Email:</strong></td><td>{form_data['customer_bill_contact_email']}</td></tr>"
            if form_data.get('customer_bill_contact_phone'):
                html_content += f"<tr><td><strong>Phone:</strong></td><td>{form_data['customer_bill_contact_phone']}</td></tr>"
            if form_data.get('customer_address'):
                html_content += f"<tr><td><strong>Address:</strong></td><td>{form_data['customer_address']}</td></tr>"
        ...
        html_content += """
            <div style="margin-top: 30px; padding-top: 10px; border-top: 1px solid #eee; color: #7f8c8d; font-size: 0.9em;">
                <p>This is an automated message. Please find complete details in the attached PDF.</p>
            </div>
        </body>
        </html>
        """
...
def project_setup_preview():
    """Show preview of project setup before submission"""
    if request.method == 'POST':
        ...
        # Handle GET request with project_id
        project_id = request.args.get('project_id')
        if not project_id:
            flash('No project data provided', 'error')
            return redirect(url_for('utilities.project_setup'))
            
        # Get project from database
        project = Project.query.get_or_404(project_id)
        # Convert project to form_data format
        form_data = {
            'company_name': project.company_name,
            'customer_name': project.customer_name,
            'customer_abbreviation': project.customer_abbreviation,
            'project_name_quickbooks': project.name,
            'customer_address': project.customer_address,
            'customer_po_number': project.po_number,
            'project_type': project.project_type,
            'project_start_date': project.start_date.strftime('%Y-%m-%d') if project.start_date else '',
            'estimated_end_date': project.end_date.strftime('%Y-%m-%d') if project.end_date else '',
            'project_status': project.status,
            'as_bid_gross_margin': str(project.gross_margin) if project.gross_margin else '',
            'customer_bill_contact_name': project.billing_contact_name,
            'customer_bill_contact_email': project.billing_contact_email,
            'customer_bill_contact_phone': project.billing_contact_phone
        }
...
def project_setup_submit():
    ...
    try:
        # Handle both direct form submission and preview submission
        if 'form_data' in request.form:
            # Coming from preview page
            import json
        ...
        project = Project(
            ...
            project_type=form_data.get('project_type'),
            start_date=datetime.strptime(form_data.get('project_start_date'), '%Y-%m-%d') if form_data.get('project_start_date') else None,
            end_date=datetime.strptime(form_data.get('estimated_end_date'), '%Y-%m-%d') if form_data.get('estimated_end_date') else None,
            status=form_data.get('project_status'),
            gross_margin=float(form_data.get('as_bid_gross_margin', 0)) if form_data.get('as_bid_gross_margin') else None,
            billing_contact_name=form_data.get('customer_bill_contact_name'),
            billing_contact_email=form_data.get('customer_bill_contact_email'),
            billing_contact_phone=form_data.get('customer_bill_contact_phone')
        )
        ...
        if not success:
            current_app.logger.error("Failed to send email with PDF")
